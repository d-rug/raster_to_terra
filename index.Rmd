...
vars = c('DataType', 'Operation', 'old', 'terra')

```


```{r run_pre_workshop, eval=FALSE}


# from geodata package
bio = worldclim_global('bio', res=0.5, path='data')
usa = gadm('USA', path='data')

writeVector(usa, 'data/usa.geojson', filetype='GeoJSON')
writeVector(usa, 'data/USA/usa.shp', filetype='ESRI Shapefile')
...
bio_fns = file.path('data/wc2.1_30s', list.files('data/wc2.1_30s'))
...
bio = worldclim_tile('bio', lon=-121, lat=38, res=2.5, path='data')
usa = gadm('USA', path='data')
r_bio = raster(bio)
sp_usa = shapefile(usa_fn)
...
writeRaster(bio, 
            filename=paste0('data/bioclim', sample.int(5e5, 1), '.GTiff'), 
            filetype='GTiff',
            overwrite=TRUE)
...
microbenchmark(
  shapefile(sp_usa, filename=paste0('data/gadm.shp', filetype='ESRI Shapefile')),
  writeVector(usa, filename=paste0('data/gadm.shp', filetype='ESRI Shapefile')),
...
microbenchmark(
  raster::projectRaster(r_bio, crs="epsg:3310"),
  terra::project(bio, "epsg:3310"),
